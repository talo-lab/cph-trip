...
/* ── 이벤트 대표 이미지 — /api/image 프록시로 공식 CDN 이미지 로드 ── */
function _loadEvImg(overlay, eventId) {
  if (!eventId) return;
  const imgEl  = overlay.querySelector('.ev-photo-img');
  const gradEl = overlay.querySelector('.ev-photo-gradient');
  if (!imgEl) return;
  fetch(`/api/image?id=${eventId}`)
    .then(r => r.ok ? r.json() : null)
    .then(d => {
      if (!d?.imageUrl) return;
      imgEl.onload  = () => imgEl.classList.add('loaded');
      imgEl.onerror = () => { imgEl.style.display='none'; if(gradEl) gradEl.style.display='none'; };
...
function openEvModal(ev, id){
  ...
  overlay.innerHTML=`
    <div class="ev-modal" id="evModal">
      ...
      <div class="ev-photo-wrap" data-dk="${_festDk}">
        ...
        <img class="ev-photo-img" alt="${ev.venue}">
  ...
  _loadEvImg(overlay, id); // 공식 사이트에서 이미지 비동기 로드
...
function openExhEvModal(ev, ex){
  ...
  overlay.innerHTML=`
    <div class="ev-modal" id="evModal">
      ...
      <div class="ev-photo-wrap" data-dk="${exhDkToPlanDk(ex.district)||''}">
        ...
        <img class="ev-photo-img" alt="${ex.brand}">
        <div class="ev-photo-gradient"></div>
      </div>
      <div class="ev-body" id="evBody">
        <div class="ev-section">
  ...
  document.body.appendChild(overlay);
  _loadEvImg(overlay, ev.id); // 공식 사이트에서 이미지 비동기 로드
